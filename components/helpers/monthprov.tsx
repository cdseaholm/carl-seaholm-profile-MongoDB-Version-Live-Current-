export async function MonthProv(monthsGiven: { month: number, year: number }[]) {
    ...
    let monthOnlyReturned = [] as string[];
    let colorsReturned = [] as string[];
    if (monthsGiven === null) {
        return { monthNames: [] as string[], monthColors: [] as string[], message: "Month or year not provided", monthNamesOnly: [] as string[] }
    } else {
        for (let i = 0; i < monthsGiven.length; i++) {
            let thisMonth = monthsGiven[i].month as number;
            if (thisMonth < 0 || thisMonth > 11) {
                return { monthNames: [] as string[], monthColors: [] as string[], message: "Month must be between 0 and 11", monthNamesOnly: [] as string[] }
                ...
                monthOnlyReturned.push(monthOnlyName);
            }
        }
    }
    return { monthNames: monthsReturned, monthColors: colorsReturned, message: 'Success', monthNamesOnly: monthOnlyReturned } as { monthNames: string[], monthColors: string[], message: string, monthNamesOnly: string[] };
}