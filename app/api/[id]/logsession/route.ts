...
export async function PUT(req: Request) {
    ...
    try {
        ...
        if (!user) {
            return NextResponse.json({ status: 404, message: "Not authorized" });
        }
        ...
        if (!newEntry) {
            return NextResponse.json({ status: 400, message: "Missing new entry" });
        } else if (!hobbyTitle) {
            return NextResponse.json({status: 404, message: "Title needed"})
        }
        ...
        if (!hobbyField) {
            return NextResponse.json({ status: 404, message: "Hobby title not found" });
        }
        ...
        return NextResponse.json({ status: 200, message: "Hobby updated successfully" });
    } catch (error: any) {
        console.error(error);
